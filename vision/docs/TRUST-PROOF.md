# Trust proof — where every mark comes from

Rule: **nothing on the page that we cannot point at a source for.** No invented
Fortune 500 logos, no made-up counts, no star ratings we don't hold.

Second rule (George, 2026-08-07): **no outbound links on any trust mark.** Clutch,
Forbes and Google are display-only. Paid traffic stays on this host.

Third rule (George, 2026-08-07): **client strip = image-backed logos only**, muted
(grayscale) for privacy. No blank wordmark slots. If Virtual Coworker has not
published a logo asset for a named client, that name stays off the strip.

## What is live

| Mark | Asset | Source |
|---|---|---|
| Clutch — Top Virtual Assistant Company, US 2024 | `brand/trust/badge-clutch-us-2024.webp` | Virtual Coworker media library |
| Clutch 4.9 / 5 · 7 reviews | text | Clutch company profile rating |
| Google 5-star reviews | `brand/trust/badge-5-star-reviews.webp` | Virtual Coworker media library |
| Forbes Business Council | `brand/badge-forbes-*.webp` | Braden Yuill's council profile |
| Founded 2011 badge | `brand/trust/badge-14-year.webp` | Virtual Coworker media library |
| Press: SMH (AU only), BRW, Startup Daily, StartupSmart | `brand/trust/press-*.webp` | "Featured In" row on virtualcoworker.com |
| Awards: Anthill Cool Company Top 100, StartupSmart Awards 2014 | `brand/trust/press-*.webp` | Same "Featured In" row |
| Client logo — College Hunks (muted, US) | `brand/trust/client-college-hunks.svg` | Client logo published on virtualcoworker.com/forbes/; US franchise (VA) |
| Client logo — Good Co. (muted, US) | `brand/trust/client-good-co.png` | Same forbes page; Laura W. / GOODco. — Charleston, SC |
| Client logo — Credit Card Compare (muted, AU+US) | `brand/trust/client-credit-card-compare.png` | Success Stories client; logo George saved 2026-08-07; also on `/us` to fill strip |
| Client logo — Buzinga Apps (muted, AU+US) | `brand/trust/client-buzinga.png` | Success Stories client; logo George saved 2026-08-07 |
| Client logo — The Learning Deli (muted, AU+US) | `brand/trust/client-learning-deli.png` | Success Stories client; icon-only cube+teardrop George ID’d 2026-08-07; strip shows caption “The Learning Deli” |
| Client logo — RecruitLoop (muted, AU+US) | `brand/trust/client-recruitloop.png` | Success Stories (Paul Slezak); logo George saved 2026-08-07 |
| Client logo — ProActive Media (muted, AU+US) | `brand/trust/client-proactive-media.png` | Success Stories (David Krynauw); logo George saved 2026-08-07 |
| Registered entities + ABN 49 154 746 004 | text, footer | Our own privacy page |
| Founder: Braden Yuill, Founder & CEO | text, footer | About page + Forbes council profile title |
| Offices (US + AU) | text, footer | Published contact details |

Industry statistics cite Deloitte GOS 2024, Bloom/Han/Liang (Nature 2024), and
industry VA rate guides for typical Philippines admin rates (~$8/hr band).
Source labels render as plain text — cited, not linked.

## What we refused to do

- **No invented big brands.** Only companies with published evidence.
- **Dropped GiggedIn, My Nappies and Allara Support Services.** All three were on
  the old roster; `giggedin.com` now redirects to an unrelated site and the other
  two no longer resolve. A dead roster reads worse than a short one.
- **Dropped Yacht and Boat, The PromoDonna, Moore Technology, Essendon Creative,
  HR Industry from the logo strip.** Named on Success Stories, but no usable logo
  file was saved for this pass.
- **Skipped a wrong Google Images grab** (`images (9).jpeg` = GoodCompany Giving
  Solutions / goodcompany.org) — not a Virtual Coworker client; not the US Good Co.
- **No inventing Fortune 500.** US-native marks (College Hunks, Good Co.) on `/us`
  only; AU success-story logos on `/au` and `/us` (George: OK to share AU on US).
- **Featured In** lives in `PressBand` below the role quiz — not inside Happy
  customers — so logos + reviews aren’t a pile.
- **No client logos scraped beyond what George saved + VC’s published /forbes/ marks.**
- **No review counts beyond the 7 on the Clutch profile.**

## Known staleness to fix at source

The "14 Years / Founded 2011" badge is the company's own 2025 artwork. The live
counter derives from the founding year, so it reads **15+** in 2026 while the
badge art still says 14. Worth asking the designer for a refreshed badge — the
copy around it uses "Since 2011", which does not go stale.

## Wishlist (not live until verified)

Tracked on Launch Control checklist (2026-08-09). Do **not** invent or scrape:

1. **US-recognizable press** — Inc, Entrepreneur, Business Insider, major US biz
   press — only if Virtual Coworker was actually featured (need article + logo OK).
2. **More authorized US client logos** — written OK from VC / client; prefer
   brands a US buyer recognizes in two seconds.
3. **Video testimonials** — face + name + company + specific outcome.
4. **Never invent Fortune 500** marks or review counts beyond Clutch’s public 7.

## Capacity urgency (live 2026-08-09)

Page-top “limited onboarding” ribbon was pulled — read cheap on desktop.
Live cue is a **red arrow pointing at the form** + “Talk to a specialist —
usually same business day.” No fake countdown / invented “spots left.”

## US vs AU trust (2026-08-09 pass 2)

- **As featured in (US):** Forbes Business Council, Clutch US, Google reviews,
  plus VC’s published Featured In row (BRW, Startup Daily, StartupSmart,
  Anthill). SMH is AU-only. Outsource Accelerator top-20 listing is real
  (VC + EIN PR, 2026) but we have no OA logo in the brand library — do not
  invent one. No Inc / Entrepreneur / BI editorial or display-ad placement found.
- **US client logos in hero:** removed. Tiny grayscale marks were broken
  (College Hunks SVG illustration + AU logos filling the US strip).
- **US TrustBand logos:** Good Co. only (Charleston, SC — published quote).
  College Hunks stays as a **named quote**, not a logo (illustration doesn’t
  read as a US brand mark).
- **AU logos** (Credit Card Compare, Buzinga, ProActive Media, Learning Deli,
  RecruitLoop) stay on `/au` only.
- **Track record stats** (15+ years, Clutch 4.9, 3 offices) sit directly under
  the hero again.
