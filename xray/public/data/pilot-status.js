...
window.PILOT_STATUS = {
  ...
  commercialLabel: "US WEBSITE CALL 60s LIVE · NEXT AU GTM/GA4 · THEN AU CALL CONVERSIONS · VC_AU_* LIVE (WAITING ON TRAFFIC) · US LIVE · BRAND PAUSED",
  objective:
  ...
  conversionStrategy: {
    stage1Primary: [
      "US website calls 60+ seconds — LIVE (Google forwarding on /us). Not a tel: tap. No CallRail required.",
      "AU prerequisite: GTM + GA4 visit tags (before AU website-call forwarding).",
      "AU website calls 60+ seconds — after AU GTM exists (public # 1300).",
      "Calls from ad call assets 60+ seconds (US live; AU still needed) — separate from website calls"
    ],
    stage1Secondary: [
    ...
    laterPath:
      "Phone is Ads conversion north star for now. After AU GTM/GA4 → AU website-call 60s → AU ad-call wins: human marks qualified in Zoho → light signal into Ads. Consult booking later (overlay is live, not bidding Primary). Later (enough deals/money): Max Conv / value-based when deals actually pay.",
    websiteCallDuration: {
      status: "us_live_au_open",
      auditedAt: "2026-08-10",
      verifiedAt: "2026-08-10",
      minSeconds: 60,
      method: "Google Ads website phone call conversion (WEBSITE_CALL) + Google forwarding number (dynamic replace for Ads visitors)",
      implementVia: "Ads UI conversion + GTM Google Tag with phone_conversion_number",
      markets: "US 888-964-8644 on site (wired) · AU 1300 886 740 on site (still needs AU action + GTM)",
      liveToday: {
        usAdCall60s: true,
        usWebsiteCall60s: true,
        usWebsiteCallAction: "VC_US_Phone_Call_From_Website",
        usWebsiteCallLabel: "Sf71CJSQr98cEOPyhMsD",
        usGtmPublished: true,
        usClickToCallSecondary: true,
        auAdCall60s: false,
        auWebsiteCall60s: false,
        micrositeForwardingTagUs: true,
        micrositeForwardingTagAu: false
      },
      notPrimary: "tel: click",
      noCallRail: true,
      testPath: "ad click → LP → forwarding number shows → call connects → 60+ sec → conversion in Ads",
      verifyNote:
        "2026-08-10: /us fires AW-962672995 + Sf71CJSQr98cEOPyhMsD WCM; visible number stayed (888) 964-8644 without real ad-click cookie (fake gclid did not swap)",
      nextStep:
        "AU order: GTM/GA4 IDs first → then AU website-call 60s + forwarding tag → then AU ad-call 60s; optional US E2E ad-click swap test",
      note: "US conversion id 7716194324 · Primary · $100 · 60s · ENABLED"
    },
  ...
  phones: {
    ...
    auDestination: {
      ...
      note: "George-approved AU site phone — live on /au. Keep answering. 1300 voicemail still says Alex (Holly / Chey / Raffie). Next: AU GTM/GA4 first → then website call 60s + forwarding → AU ad-call wins → Unkown AG cleanup."
    },
    callRail:
  ...
  confirmedComplete: [
    ...
    {
      id: "website_call_duration_us",
      label: "US website calls 60+ sec live — VC_US_Phone_Call_From_Website · Primary · GTM published · tags fire on /us",
      status: "complete"
    }
  ],
  verifyNow: [
    {
      ...
      label: "US ad-call + US website-call Primary live; form Secondary; Zoho Qualified offline later — no legacy Zoho/Zapier; don’t invent IDs",
      status: "verify"
    },
    {
  ...
  waitingOnVc: [
    ...
    {
      id: "au_gtm_ga4",
      label: "AU GTM + GA4 visit tags (prerequisite before AU website-call forwarding) — campaigns already live",
      status: "waiting"
    },
    {
      id: "au_website_call_60s",
      label: "AU website calls 60+ sec via Google forwarding (after AU GTM) — public # stays 1300",
      status: "waiting"
    },
    {
      id: "au_ad_call_60s",
      label: "AU calls from ads 60+ sec conversion action (separate from website calls)",
      status: "waiting"
    },
    {
  ...
  georgeHandles: [
    "US website calls 60s — done; next AU GTM/GA4 first, then AU call conversions",
    "AU order: GTM/GA4 → website-call 60s → ad-call 60s → then Zoho qualified → Ads; Unkown AG cleanup",
    ...
    "After AU GTM + AU call conversions: Zoho qualified → Ads; value-based later",
  ...
  majorBlockers: [
    "AU GTM + GA4 visit tags (prerequisite — before AU website-call forwarding)",
    "AU website calls 60s + Google forwarding (after AU GTM; VC_AU_* already Enabled)",
    "AU calls from ads 60s (separate conversion action)",
    ...
    "After AU call tracking: Zoho qualified → Ads; later value when deals make money",
    "US inbox monitoring awaiting Cheyenne (APAC/Holly watches apac@)"
  ],
  nextThree: [
    "Australia — website tags (GTM + GA4) first",
    "Then Australia — website calls 60s · separately calls from ads 60s",
    "Then Zoho qualified mark → Ads; site tests stay at the bottom until working"
  ],
  openItemsUnresolved: [
    "AU GTM/GA4 website tags (prerequisite before AU website-call forwarding)",
    "AU website calls 60+ sec via Google forwarding (after AU GTM) — not tel: click Primary; no CallRail needed",
    "AU phone call conversions from ads (separate; before Zoho → Ads)",
    "AU Unkown AG / Broad-positive cleanup",
    "Site tests GTM/GA4 experiment wiring (bottom until working)",
    "Zoho qualified → Ads path (after AU GTM + AU call conversions)",
