...
- **Later (deferred during cold start, 2026-08-14):** Zoho → Google Ads offline conversions so campaigns learn which leads produce business. Not cancelled. Not this week. Revisit only after the five-item gate in `ads-launch/ZOHO-COLD-START-DEFERRED-2026-08-14.md`.
...
|--------|--------|--------|
| US | **Hot ops** | Destination `888-964-8644` — verified VC US line (George 2026-08-10 restore). **Cheyenne owns US sales.** Raffie = phone systems/IT, **not** US salesperson. Real task: confirm answer path + missed-call owner + E2E test. |
...
- **Preferred (when Zoho path locked — after the cold-start revisit gate):** new LP form → secure server-side → direct Zoho + success/fail + backup. No browser credentials.
- **Zoho:** George can sign in. Mapping / admin / direct write / offline / Job Order later. **Deferred during cold start** — not cancelled, not a traffic-readiness blocker, not this week’s Ads optimization work. Don’t rebuild Zoho as the week’s center.
...
7. Zoho integration added later without rebuilding the form — only after the cold-start revisit gate
...
| `NEXT_PUBLIC_US_PHONE` | Site default: `888-964-8644` / `tel:+18889648644` (George 2026-08-10 restore) |
