...
Operator next: clear **TRAFFIC READY** (durable channel + test + responder) → **prefer** one custom domain + Final URLs on that host → Import split CSVs → review Paused → Post Paused → Enable Tier 1A/1B only with explicit George OK (if imported on preview, rewrite Final URLs before Enable). Campaign-specific goals + Zoho discovery = parallel later.
